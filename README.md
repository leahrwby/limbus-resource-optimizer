...
- 未提供剧情进度时，Skill 默认玩家已经解锁困难镜牢。
- 一旦用户说明当前章节或关卡，就按实际进度判断，不再套用默认值。
- 当前普通镜牢需通关第二章后开启；困难镜牢需通关第八章，并先完整通关一次当前普通镜牢。
- 若用户只说“处于第二章”，普通镜牢可能仍未开放。Skill 会先给出通关第二章所需体力的范围，再比较“通关第二章后刷普牢”和“继续推进到第八章后刷困牢”。
- 支持 `3-5` 这类具体关卡表达，并固定解释为“3-5 尚未通关”，从该关开始计入首次通关消耗；零体力剧情节点也能用于定位进度。
- 没有具体关卡时按范围估算；没有规划天数时不会武断地宣布哪条路线绝对更优。

示例提示词：

```text
我处于第二章。请比较未来30天内，先通关第八章开启困牢，和通关第二章后直接刷普牢，哪条路线收益更高。
```
...
- 用户只说“一般人格”或“一个人格”时，默认按400人格碎片兑换1个人格计算；只有明确指定二星人格或其他价格时才改用对应成本
